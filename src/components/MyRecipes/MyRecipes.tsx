...
import CommonSkeleton from "../CommonSkeleton/CommonSkeleton";
...
const MyRecipes = () => {
  ...
  const { data: recipe, isLoading } = useGetUserRecipesQuery(
    {
      query: {
        page,
        search: searchTerm,
  ...
  return (
    <div className="w-full flex flex-col justify-center gap-10 items-center p-8">
      <h4 className="font-semibold text-2xl flex justify-start items-start w-full">
      ...
      <div className="w-full  p-10 border-2 border-dashed">
        ...
        <div className="w-full bg-[#fbfcfd] shadow-sm min-h-[40vh]">
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              ...
              {recipe?.data?.map((item: any) => (
                <TableRow key={item?._id}>
                  ...
                  <TableCell>
                    <span className="text-green-500 font-semibold"></span> Ready
                    In {item?.readyIn} (minutes)
                  </TableCell>
                  <TableCell>
